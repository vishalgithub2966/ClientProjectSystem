...
RoxyUtils.FormatDate = function (myDate) {
    var ret = '';
    try {
        ret = myDate.getFullYear() + '-' + ('0' + (myDate.getMonth() + 1)).slice(-2) + '-' + myDate.getDate() + ' ' + myDate.getHours() + ':' + ('0' + (myDate.getMinutes())).slice(-2); //+ ':' + myDate.getSeconds()
...
RoxyUtils.GetFileIcon = function (path) {
    var ret = '/lib/fileman/images/filetypes/unknown.png';//'images/filetypes/file_extension_' + RoxyUtils.GetFileExt(path).toLowerCase() + '.png';
    if (RoxyUtils.GetFileExt(path).toLowerCase()) {
        ret = RoxyUtils.GetFileExt(path).toLowerCase();
        ret = RoxyUtils.FileIconClass(ret);
    }
    return ret;
};
RoxyUtils.FileIconClass = function (path) {
    ...
    else if (ext == "zip" || ext == "rar" || ext == "s7z" || ext ==  "b6z") {
        return "archive-o";
    }
    else if (ext == "mp3" || ext == "m4p" || ext == "sln" || ext ==  "raw") {
        return "audio-o";
    }
    else if (ext == "html" || ext == "aspx" || ext == "php" || ext == "java" || ext ==  "xml") {
        return "code-o";
    }
    else if (ext == "xls" || ext == "xlt" || ext ==  "xlm") {
        return "excel-o";
    }
    else if (ext == "avi" || ext == "3gp" || ext == "gifv" || ext == "flv" || ext == "MTS" || ext ==  "M2TS") {
        return "video-o";
    }
    else if (ext == "pdf") {
        return "pdf-o";
    }
    else if (ext == "pptx" || ext == "pot" || ext ==  "ppt") {
        return "powerpoint-o";
    }
    else if (ext == "txt") {
        return "text-o";
...
RoxyUtils.LoadConfig = function () {
    $.ajax({
        url: '/lib/fileman/conf.json',
        dataType: 'json',
        async: true,
        success: function (data) {
            RoxyFilemanConf = data;
            abp.event.trigger('app.triggerConfigLoad');
        },
        error: function (data) {
...
RoxyUtils.LoadLang = function () {
    ...
    if (RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto') {
        if (RoxyUtils.GetUrlParam('langCode')) {
            langUrl = '/lib/fileman/lang/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json';
        }
        else {
            var language = window.navigator.userLanguage || window.navigator.language;
            langUrl = '/lib/fileman/lang/' + language.substr(0, 2) + '.json';
        }
        ...
        if (RoxyFilemanConf.LANG) {
            langUrl = '/lib/fileman/lang/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json';
            if (!RoxyUtils.FileExists(langUrl))
                langUrl = '';
        }
    }
    if (!langUrl)
        langUrl = '/lib/fileman/lang/en.json';
